...
{
  Implements a simple linked list class. For a 'full fat' one that is comparable
  to the stock, array-based TList and TObjectList, see CCR.LinkedList.pas.
}
interface
...
uses System.SysUtils, System.RTLConsts; //for the standard exception types and error messages
...
{ TSimpleLinkedList<T>.TEnumerator }

constructor TSimpleLinkedList<T>.TEnumerator.Create(AHead: TNode);
begin
  FDoneFirst := False;
  FNode := AHead;
end;

function TSimpleLinkedList<T>.TEnumerator.MoveNext: Boolean;
begin
...
function TSimpleLinkedList<T>.TEnumerator.GetCurrent: T;
begin
  Result := FNode.Value;
end;
...
function TSimpleLinkedList<T>.GetEnumerator: TEnumerator;
begin
  Result := TEnumerator.Create(First);
end;
